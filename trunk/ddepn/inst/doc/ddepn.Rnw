...
\caption{Result plot of the genetic algorithm. Left: the given graph; Middle: the inferred graph; 
...
if(require(multicore)) {
	ret <- ddepn(data, phiorig=net, inference="mcmc",
		maxiterations=mi, burnin=burnin,
...
package includes a snapshot of the KEGG database (containing signalling and disease pathways),
downloaded in October 2010.
...
an approach of \cite{Werhli2007} is followed. The total number of node pairs $M$
occurring in all reference networks is counted as well as the number of node pairs $m$ that are connected 
via an edge in all reference networks. The prior probability matrix $\mathcal{B}$ is defined as:

\[ \mathcal{B} = \frac{m}{M}, \]

where each entry is multiplied with $-1$ if the edge occurred as inhibiting edge in the KEGG 
pathways. This kind of prior matrix can the be used together with the \emph{laplaceinhib} prior:
...
\emph{laplace} prior type. Here, all entries in $\mathcal{B}$ are positive, and for the prior 
calculation, the edge type in the inferred network is ignored.
