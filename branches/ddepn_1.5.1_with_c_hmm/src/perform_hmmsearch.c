    ...
    const int T, const int R, int M, const int hmmit)
{
  ...
  float temp = 1.0;
  float infinity = -1 * (temp / (temp - 1.0));
  ...
  double Lik = infinity, Lold, diff, diffsold[hmmit];
  ...
  while (it <= hmmit)
    {
      ...
      for (int t = 0; t != T; ++t)
        { // all time points
          double maxemission = infinity, maxemissionbefore = infinity;
          for (int m = 0; m != M; ++m)
            { // all states in gamma
              int ind = (int)(t*M+m);
              if (t == 0)
                {
                  //initialise
                  viterbi[ind] = -log2(M) + getE(X, t, Gsub, m,
                      THsub, N, R);
                 }
              else
                {
                  viterbi[ind] = maxemissionbefore + getE(X, t,
                      Gsub, m, THsub, N, R);
                }
  ...
  for (t = 0; t != (M * M); t++)
    {
      ...
      if (Anum[t] != 0)
        {
          A[t] = log2(Anum[t]) - log2(Adenom[(t % M)]);
          Asum += pow(2, A[t]);
        }
    }
